...
var Page={
  ...
  setPageInfo:function(page){
    ...
    Init:function(){
      $('form[name=pageForm]').append('<input type="hidden" class="pageInput" name="currentPageNo" value="">');
      $(document).on('click', '.btn_p_prev_no', function(event) {
        $('.pageInput').val(1);
        if($('.btn_subnmit').length>0){$('.btn_subnmit').click();}else{$('form[name=pageForm]').submit();}
